...
export default function Panel({sdk,context,ui}) {
 ...
 async function shell(args){const r=await sdk.runShell({summary:'Compile approved captions',command:'python3 "$SELECTS_USER_PANELS_ROOT/stylish-captions/approved/compile-captions.py" '+args,timeoutMs:300000,maxOutputBytes:48000});if(r.isError||r.exitCode!==0){const detail=(r.stderr||r.output||'').match(/(?:ValueError|AssertionError): ([^\n]+)/);throw Error(detail?detail[1]:"The caption renderer could not complete this version. Check the DOAC Style installation, then try again.");}return r.stdout;}
 ...
 const codeResult=await sdk.runShell({summary:'Read caption renderer',command:'cat "$SELECTS_USER_PANELS_ROOT/stylish-captions/approved/caption-scene.tsx.txt"',maxOutputBytes:16000});if(codeResult.isError||codeResult.exitCode!==0)throw Error('The caption renderer is missing. Reinstall DOAC Style.');
