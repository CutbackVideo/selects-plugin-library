...
3. Resolve the current user's home directory through the host FileSystem service or the operating system. Create `.selects/templates/place-stories` beneath that home directory. Copy these four files there from the downloaded package:
...
4. Read `panel.tsx` and register it through `save_panel({name: "place-stories", source})`. Read and preserve an existing panel before replacing it. If an older local prototype is also installed, leave it intact unless the user asks to retire it.
5. Open **Place Stories** in a project. Confirm that the interface is English and no missing-assets or host-adapter warning appears. Choose a small location folder, preview its clips, and check the title and description fields.
...
The plugin creates its own project-scoped data under `.selects/plugin-data/place-stories` beneath the user's home. It does not require a pre-created source-media folder. Keep recovery and history data local and out of public source control.
