...
const story={settings:{pace:'balanced',intro:true},places:[makePlace(1),makePlace(2)]};const quickPlan=box.plan({...story,settings:{...story.settings,pace:'quick'}});assert.equal(quickPlan.segments.filter(s=>s.place===0).length,1);const plan=box.plan(story);assert.equal(plan.places.length,2);assert.equal(plan.segments.length,4);assert(Math.abs(plan.seconds-9.4)<1e-8);assert(plan.opening);assert.equal(story.places[0].picks[0].end,6);const three={settings:{pace:'quick',intro:false},places:[{...makePlace(1),picks:[...makePlace(1).picks,{path:'/raw/1c.mp4',start:1,end:6,cropX:.5,cropY:.5}],files:[...makePlace(1).files,{path:'/raw/1c.mp4',duration:8}]}]};assert.equal(box.plan(three).segments.filter(s=>s.place===0).length,3);
const first=plan.segments[0];assert.notEqual(first.path,plan.segments[1].path);assert(plan.segments.slice(1).filter(s=>s.path===first.path).every(s=>s.end<=first.start||s.start>=first.end));assert.throws(()=>box.plan({settings:story.settings,places:[{...makePlace(1),picks:[{path:'/foreign',start:0,end:1}]}]}));
