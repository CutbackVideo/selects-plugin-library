...
for(const r of cfg.regions){
 ...
 await d.addMotionGraphic({label:intro?'Opening title':'Place '+String(r.place+1).padStart(2,'0'),within:await d.rangeAtFrames(r.startFrame,r.endFrame),tsxCode:cfg.motionSource,parameters:{kind:intro?'intro':'place',title:intro?cfg.title:place.name,description:intro?cfg.subtitle:place.description,index:r.place+1,count:cfg.places.length,accent:cfg.accent||'#f8d103',titleFont:'Times New Roman'},editableParameters:[{key:'title',label:intro?'City':'Place name',type:'text',defaultValue:''},{key:'description',label:intro?'Country or region':'Short note',type:'text',defaultValue:''},{key:'accent',label:'Title color',type:'color',defaultValue:'#f8d103'},{key:'titleFont',label:'Title font',type:'text',defaultValue:'Times New Roman'}]});
}
