...
const newStory=pid=>({version:VERSION,projectId:pid,id:uid(),title:'',subtitle:'',folder:'',places:[],settings:{aspect:'portrait',pace:'quick',intro:true,assist:true,sourceAudio:false,music:null,themeMusic:true,accent:'#f8d103'},job:null,notices:[]});
...
// cut is a whole number of beats after it.
// Renders are cached by this name: change it whenever the theme file changes.
const THEME_VERSION='theme-v1-48k',THEME_FILE='place-count-theme.m4a',THEME_BAR=4*60/92,THEME_FADE=1.5;
function encodeWav(buffer){const ch=buffer.numberOfChannels,rate=buffer.sampleRate,n=buffer.length,view=new DataView(new ArrayBuffer(44+n*ch*2)),text=(o,v)=>{for(let i=0;i<v.length;i++)view.setUint8(o+i,v.charCodeAt(i));};text(0,'RIFF');view.setUint32(4,36+n*ch*2,true);text(8,'WAVE');text(12,'fmt ');view.setUint32(16,16,true);view.setUint16(20,1,true);view.setUint16(22,ch,true);view.setUint32(24,rate,true);view.setUint32(28,rate*ch*2,true);view.setUint16(32,ch*2,true);view.setUint16(34,16,true);text(36,'data');view.setUint32(40,n*ch*2,true);const data=[...Array(ch)].map((_,c)=>buffer.getChannelData(c));let o=44;for(let i=0;i<n;i++)for(let c=0;c<ch;c++){const v=Math.max(-1,Math.min(1,data[c][i]));view.setInt16(o,v<0?v*32768:v*32767,true);o+=2;}return new Uint8Array(view.buffer);}
...
function planStory(story) {
  ...
  places.forEach((place,index) => {
    ...
    const slots = count === pattern.length && beats === PLACE_BEATS[pace]
      ? pattern
