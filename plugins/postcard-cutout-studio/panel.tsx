...
const DEFAULTS={subjectId:'',subjectStartSec:0,bgIds:['','',''],photoIds:['','',''],title:'SWITZERLAND',subtitle:'moving postcards from',fontFamily:'DIN Condensed',titleColor:'#ed0000',subtitleColor:'#f5f3f5',aspect:'landscape',fitMode:'cover',forceNew:false,autoExport:false};
...
let heldSubject=null,cutoutInput=null,madeForeground=false;
if(kind!=='resume'&&kind!=='export'){
 ...
 const panelHolds=s.bgIds.map((id,i)=>{const row=byId.get(id);return row?.path&&isVideo(row)?helper(sdk,'hold',{path:row.path,start:0,target:Math.max(.5,TIMING.stripEnd-(TIMING.stripFirst+TIMING.stripEvery*i))+.25,silent:true}):null;});
 ...
 const missing=[...selected.map(row=>row.path),...(cutoutInput?.path?[cutoutInput.path]:[])].filter(path=>!inventory.some(item=>item.path===path));
 if(missing.length){
 ...
 if(previous?.draftId&&requestKey(s)===requestKey(previous.settings)){setStatus('Your postcard is ready to play and edit.');return;}
...
if(current.phase==='maskReady'){guard(pid);const [fg,draftName]=await Promise.all([prepareForegroundResource(sdk,current,madeForeground),uniqueDraftName(sdk,pid,current.settings.title)]);guard(pid);await ensured;current=await claim(current,['maskReady'],{phase:'assemblySubmitting',assemblyStartedMs:Date.now(),foregroundPath:fg.foregroundPath,foregroundVersion:fg.foregroundVersion,foregroundResourceId:fg.foregroundResourceId,sfxIds:fg.sfxIds,draftName},'assembly',{foregroundResourceId:fg.foregroundResourceId,draftName});const script=makeAssembly(current);const result=await runScript(sdk,script,'Create postcard with one commit',true);current=await persist(current,{phase:'draftReady',draftId:result.draftId,assembly:result},'assembly','end',{durationMs:Date.now()-current.assemblyStartedMs,result,trace:window.__postcardTrace||[]});}
...
function setSelected(ids,preferred=s.subjectId){
  ...
  // Only the subject has to be footage: its cutout is what moves. The panels
  // overlay and scale a resource, and the ending cuts 0.2s slices out of one —
  // neither asks whether it plays, so either kind fills either role. Video
